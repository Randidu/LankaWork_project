...
from app.schemas.rating import RatingCreate, RaingResponse
from app.services import rating_service

router = APIRouter(prefix="/ratings", tags=["Ratings"])

@router.post("/", response_model=RaingResponse)
def create_rating(rating: RatingCreate, db: Session = Depends(get_db)):
    return rating_service.create_rating(db, rating)

@router.get("/worker/{worker_id}", response_model=list[RaingResponse])
